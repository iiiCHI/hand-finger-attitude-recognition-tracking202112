%% 这个文件是处理第一个实验数据的函数，用于计算第一个实验产生的姿态，得到第一个实验的姿态以后，进入data的实验一执行statistics文件
clc;clear;
%% 定义接入程序的IMU硬件数目
IMU_num = 8;
PI = 3.1415926;
...
%% 数据处理
for i = 0:8
    for Flagiii = 0:1 %只有1是加约束，其余是不加约束
        Index_Tilt = floor(i/3)+1;
        Index_Bend = mod(i,3)+1;
        % Flagiii = mod(i,2);%奇数加约束，偶数不加约束   
        ReadFileName = "D:\MATLAB\WorkSpace\MaYongWei\A实验_角度对比\有无约束系统的对比\data\实验1\原始数据"+num2str(Tilt(Index_Tilt))+"\";
        WriteFileName = "D:\MATLAB\WorkSpace\MaYongWei\A实验_角度对比\有无约束系统的对比\data\实验1\"+num2str(Tilt(Index_Tilt))+"\";
        if Flagiii == 1
