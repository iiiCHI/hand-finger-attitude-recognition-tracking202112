...
% Flagiii = 0; %只有1是加约束，其余是不加约束
% IdNumber = 2;%第几个人的
for IdNumber = 2:7
    for Flagiii = 0:1
        %% 获取实验目录
        FileName = ".\data\实验2\"+string(IdNumber);
        RawFile = dir(FileName); %主要是这个结构，可以提取所有文件
        AllFile = string(sort(split([RawFile([RawFile.isdir]==0).name],".csv")));
        
        ReadFileName = string([]);
        WriteFileName = ".\data\实验2\结果\MeanAnswerId"+string(IdNumber-1)+".xls";
        for i = 2:size(AllFile)
            ReadFileName(end+1,:) = AllFile(i);
        %     WriteFileName = ".\data\实验2\结果\"+num2str(Tilt(Index_Tilt))+"\";    
        end
        FileNumberSum = length(ReadFileName);%表示一共采集的多少条数据
        for FileNumber = 1:FileNumberSum
            % 前FileNumberSum个，即为抓握所求
            Data_row = xlsread(FileName+ "\" + ReadFileName(FileNumber)+".csv");
            %% 开始准备
            format long g;
            count=1;
            buffer = [];
        ...
        %     disp(['倾斜角度:',num2str(Tilt(Index_Tilt)),',弯曲角度:',num2str(Bend(Index_Bend)),',平均值为:',num2str(mean(Cos_angle)),'°,均方误差为:',num2str(Rmse),',标准差为:',num2str(std(Cos_angle))])
            xlswrite(WriteFileName,mean(abs(Hand_posture)),'Sheet1',"A"+string(FileNumber+FileNumberSum*Flagiii));
            xlswrite(".\data\实验2\结果\QuaternionResultId"+string(IdNumber-1)+".xls",Hand_posture,"Sheet"+string(Flagiii+1),"A"+string(All_number*(FileNumber-1)+1));
            disp("成员编号:"+string(IdNumber-1)+"  是否约束："+string(Flagiii)+"  文件名："+string(FileNumber)+"   Over")
            fclose('all');
            %关闭文件
            % disp('End...');
        end 
