...
IMU_num = 3;
datafilename='New Filef4.csv';
before = [];
fid=fopen(datafilename);       %首先打开文本文件coordinate.txt
while ~feof(fid)    % while循环表示文件指针没到达末尾，则继续
    % 每次读取一行, str是字符串格式
    str = fgetl(fid);     
    
    % 以 ',' 作为分割数据的字符,结果为cell数组
    s=str2num(char(regexp(str,' ','split')))';
    if size(s,2) ~= 73 || sum(abs(s)) > 20000
        continue
    end
