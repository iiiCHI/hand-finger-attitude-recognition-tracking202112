...
ReadFileName = "D:\MATLAB\WorkSpace\MaYongWei\三指8IMU计算\data\对比结果\"+num2str(0)+"\"+num2str(30)+".csv";
Flagiii = 1;
...
Hand_posture = zeros(size(Data_row,1)+2,7*4);%用于存放 手部姿态，拇指两个关节姿态，食指两个关节姿态，中指两个关节姿态
%% 开始准备
% ti=tic;
format long g;
% 
...
%         options=optimoptions('fmincon', 'Display', 'off', 'Algorithm', 'sqp-legacy');% interior-point % sqp
        if Flagiii == 1
...
a = asin(Hand_posture(:,4*4-2))/pi*360;
% a = acos(Hand_posture(:,4*4-3))/pi*360;
disp(['平均值为:  ',num2str(mean(a))]);
disp(['标准差为:  ',num2str(std(a))]);
