function mag_calibration(before,IMU_num)
%% 函数是对于磁力计进行校准
% 输入变量：
%       before：磁力计数据，大小应为[m,(IMU_num*9+1)]
%       IMU_num：传感器的数量
% 输出变量：
%       名为Xishu.m的文件，保存NiheA的变量[x0,y0,z0,xScale,yScale,zScale]
% 作者：马永伟

%datafilename='../data/multi_IMUs_9dofdata_beforeCalib35937387949.csv';
...
for N = 1:IMU_num
    ...
    x_column = (N-1)*9+7;%7
    y_column = x_column+1;%8
    z_column =x_column+2;%9
    ...
    hold on;
    plot3(real(x),real(y),real(z),'g');
    % scatter3(x,y,z,'g');
    
    %fprintf('IMU_'+N+'_拟合结果\n');
    fprintf('IMU__拟合结果\n');
    fprintf('a = %f, b = %f, c = %f, d = %f, e = %f, f = %f\n',resoult);
    NiheA(1,N) = x00;%将系数存储到数组之中
    NiheA(2,N) = y00;
    NiheA(3,N) = z00;
    NiheA(4,N) = xScale;
    NiheA(5,N) = yScale;
