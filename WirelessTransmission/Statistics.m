...
Flagiii = 1; 
ReadFileName = "D:\MATLAB\WorkSpace\MaYongWei\三指8IMU计算\data\四元数对比\"+num2str(Tilt(Index_Tilt))+"\"+num2str(Bend(Index_Bend))+".csv";
...
%% 四元数表示方法
Error = abs(Hand_posture(:,4*4-3:4*4)) - [cos(Bend(Index_Bend)/360*pi),sin(Bend(Index_Bend)/360*pi),0,0];
Rmse = sqrt(dot(Error,Error)/size(Error,1));
disp(['倾斜角度:',num2str(Tilt(Index_Tilt)),',弯曲角度:',num2str(Bend(Index_Bend)),',平均值为:',num2str(mean(Hand_posture(:,4*4-3:4*4))),',均方误差为:',num2str(Rmse)]);
